...
openconnect_binary=${OPENCONNECT_BINARY:-"$openconnect_prefix/bin/openconnect"}
...
[[ -x "$openconnect_binary" ]] || { echo "OpenConnect executable not found: $openconnect_binary" >&2; exit 1; }
...
cp -fL "$openconnect_binary" "$binary_dir/openconnect"
cp -fL "$vpnc_script" "$runtime/vpnc-script"
chmod 755 "$binary_dir/openconnect" "$runtime/vpnc-script"
...
while IFS= read -r dependency; do queue_library "$dependency"; done < <(otool -L "$openconnect_binary" | tail -n +2 | awk '{print $1}')
...
install_name_tool -add_rpath '@executable_path/../../../Frameworks' "$binary_dir/openconnect" 2>/dev/null || true
for binary in "$binary_dir/openconnect" "$frameworks"/*.dylib; do
  [[ -f "$binary" ]] || continue
...
echo "Embedded OpenConnect plus ${#queue[@]} runtime libraries."
